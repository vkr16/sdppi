...
<body>
            ...
            <div class="w-100">
                <div class="container-fluid">
                    <div class="col-md-12 row ps-3 py-3 my-2">
                        <span class="col-md-7">
                            <p class="fs-3 fw-normal m-0">Manajemen Data Penyelenggara Telekomunikasi</p>
                        </span>
                        <nav aria-label="breadcrumb" class="m-0 col-md-5 d-flex align-items-center justify-content-end">
                            <ol class="breadcrumb m-0 ">
                                ...
                                <li class="breadcrumb-item active" aria-current="page">Telekomunikasi</li>
                            </ol>
                        </nav>
                    </div>
                        ...
                        <div class="col-12 mb-4">
                            <div class="card">
                                <div class="card-header bg-light-kominfo d-flex justify-content-between">
                                    <span><i class="fa-solid fa-tower-cell fa-fw"></i> &nbsp; Data Penyelenggara Telekomunikasi</span>
                                    <a href="penyelenggara?x=Telekomunikasi" class="btn btn-sm btn-danger"><i class="fa-solid fa-trash-alt"></i> &nbsp; Bulk Delete</a>
                                </div>
                                <div class="card-body table-responsive" style="min-height: 30vh">
                                    <table id="telekomunikasi-table" class="table table-hover">
                                        ...
                                        <tbody>
                                            <?php foreach ($telekomunikasiDataArray as $index => $telekomunikasi) { ?>
                                                <tr>
                                                    <td role="button" onclick="openUpdateDataModal(<?= $telekomunikasi['id']; ?>)"></td>
                                                    <td role="button" onclick="openUpdateDataModal(<?= $telekomunikasi['id']; ?>)"><?= $telekomunikasi['nama']; ?></td>
                                                    <td role="button" onclick="openUpdateDataModal(<?= $telekomunikasi['id']; ?>)"><?= $telekomunikasi['jenis_layanan']; ?></td>
                                                    <td role="button" onclick="openUpdateDataModal(<?= $telekomunikasi['id']; ?>)"><?= $telekomunikasi['domisili']; ?></td>
                                                    <td role="button" onclick="openUpdateDataModal(<?= $telekomunikasi['id']; ?>)"><?= $telekomunikasi['status']; ?></td>
                                                    <td><a class="text-decoration-none" href="http://<?= $telekomunikasi['website']; ?>" target="_blank"><?= $telekomunikasi['website']; ?></a></td>
                                                </tr>
                                            <?php } ?>
                                        </tbody>
                                    </table>
                                    ...
                                    <div class="card-body">
                                        ...
                                        <div class="tab-content" id="nav-tabContent">
                                            ...
                                            <div class="tab-pane fade" id="nav-batch" role="tabpanel" aria-labelledby="nav-batch-input-tab" tabindex="0">
                                                <form action="telekomunikasi/batchUpload" method="POST" class="mt-4" enctype="multipart/form-data">
                                                    <div class="mb-3">
                ...
                <div class="modal-dialog">
                    <div class="modal-content editDataPenyelenggara">
                        ...
                        <div class="modal-footer d-flex justify-content-between">
                            <button class="btn btn-danger" onclick="deleteDataTelekomunikasi()"> <i class="fa-solid fa-trash-can fa-fw"></i>&nbsp; Delete</button>
                            <div>
                                <button type="button" class="btn btn-secondary" data-bs-dismiss="modal">Batal</button>
                                <button class="btn btn-kominfo" onclick="updateDataTelekomunikasi()"><i class="fa-solid fa-floppy-disk fa-fw"></i> &nbsp;Simpan Perubahan</button>
                            </div>
    ...
    <script>
        ...
        $('#sidebar-item-telekomunikasi').addClass('sidebar-active').removeClass('sidebar-item');
        ...
        function singleInput() {
            ...
            if (nama == '' || jenis == '' || domisili == '' || status == '' || website == '') {
                ...
                $.post("telekomunikasi/singleInput", {
                        nama: nama,
                        jenis: jenis,
                        domisili: domisili,
                        status: status,
                    ...
                    .done(function(data) {
                        if (data == "success") {
                            ...
                            setTimeout(() => {
                                location.replace('<?= HOST_URL ?>/admin/telekomunikasi');
                            }, "1000")
                        }
                    });
            }
        ...
        function openUpdateDataModal(id) {
            ...
            $.post("telekomunikasi/getSingleData", {
                    id: id
                })
                .done(function(data) {
        ...
        function updateDataTelekomunikasi() {
            var id = $('#updateid').val()
            ...
            if (nama == '' || jenis == '' || domisili == '' || status == '' || website == '') {
                ...
                $.post("telekomunikasi/updateData", {
                        id: id,
                        nama: nama,
                        jenis: jenis,
                        domisili: domisili,
        ...
        function deleteDataTelekomunikasi() {
            var id = $('#updateid').val()
            var nama = $('#updatenamapenyelenggara').val()

            Notiflix.Confirm.show(
                'Konfirmasi Penghapusan',
                'Apakah anda ingin menghapus ' + nama + ' dari daftar penyelenggara telekomunikasi?',
                'Ya',
                'Tidak',
                () => {
                    Notiflix.Block.dots('body', 'Syncronizing....')
                    $.post("telekomunikasi/deleteData", {
                            id: id,
                        })
                        .done(function(data) {
                            if (data == 'deleted') {
