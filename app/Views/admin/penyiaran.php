...
<body>
            ...
            <div class="w-100">
                <div class="container-fluid">
                    <div class="col-md-12 row ps-3 py-3 my-2">
                        <span class="col-md-7">
                            <p class="fs-3 fw-normal m-0">Manajemen Data Penyelenggara Penyiaran</p>
                        </span>
                        <nav aria-label="breadcrumb" class="m-0 col-md-5 d-flex align-items-center justify-content-end">
                            <ol class="breadcrumb m-0 ">
                                ...
                                <li class="breadcrumb-item active" aria-current="page">Penyiaran</li>
                            </ol>
                        </nav>
                    </div>
                                    ...
                                    <table id="penyiaran-table" class="table table-hover">
                                        ...
                                        <tbody>
                                            <?php foreach ($penyiaranDataArray as $index => $penyiaran) { ?>
                                                <tr>
                                                    <td role="button" onclick="openUpdateDataModal(<?= $penyiaran['id']; ?>)"></td>
                                                    <td role="button" onclick="openUpdateDataModal(<?= $penyiaran['id']; ?>)"><?= $penyiaran['nama']; ?></td>
                                                    <td role="button" onclick="openUpdateDataModal(<?= $penyiaran['id']; ?>)"><?= $penyiaran['jenis_layanan']; ?></td>
                                                    <td role="button" onclick="openUpdateDataModal(<?= $penyiaran['id']; ?>)"><?= $penyiaran['domisili']; ?></td>
                                                    <td role="button" onclick="openUpdateDataModal(<?= $penyiaran['id']; ?>)"><?= $penyiaran['status']; ?></td>
                                    ...
                                    <div class="card-body">
                                        ...
                                        <div class="tab-content" id="nav-tabContent">
                                            ...
                                            <div class="tab-pane fade" id="nav-batch" role="tabpanel" aria-labelledby="nav-batch-input-tab" tabindex="0">
                                                <form action="penyiaran/batchUpload" method="POST" class="mt-4" enctype="multipart/form-data">
                                                    <div class="mb-3">
                ...
                <div class="modal-dialog">
                    <div class="modal-content editDataPenyelenggara">
                        ...
                        <div class="modal-footer d-flex justify-content-between">
                            <button class="btn btn-danger" onclick="deleteDataPenyiaran()"> <i class="fa-solid fa-trash-can fa-fw"></i>&nbsp; Delete</button>
                            <div>
                                <button type="button" class="btn btn-secondary" data-bs-dismiss="modal">Batal</button>
                                <button class="btn btn-kominfo" onclick="updateDataPenyiaran()"><i class="fa-solid fa-floppy-disk fa-fw"></i> &nbsp;Simpan Perubahan</button>
                            </div>
    ...
    <script>
        ...
        $('#sidebar-item-penyiaran').addClass('sidebar-active').removeClass('sidebar-item');
                ...
                $.post("penyiaran/singleInput", {
                    ...
                    .done(function(data) {
                        if (data == "success") {
                            ...
                            setTimeout(() => {
                                location.replace('<?= HOST_URL ?>/admin/penyiaran');
                            }, "1000")
                        }
                    });
            }
        ...
        function deleteDataPenyiaran() {
            var id = $('#updateid').val()
            var nama = $('#updatenamapenyelenggara').val()

            Notiflix.Confirm.show(
                'Konfirmasi Penghapusan',
                'Apakah anda ingin menghapus ' + nama + ' dari daftar penyelenggara penyiaran?',
                'Ya',
                'Tidak',
                () => {
                    Notiflix.Block.dots('body', 'Syncronizing....')
                    $.post("penyiaran/deleteData", {
                            id: id,
                        })
                        .done(function(data) {
                            if (data == 'deleted') {
